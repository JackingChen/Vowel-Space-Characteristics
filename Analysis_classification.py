...
columns=[
    ...
    'Proximity[pear_12]', 'Proximity[spear_12]', 'Proximity[kendall_12]',
    'Proximity[dcorr_12]',
]
...
# for singleFeat in columns:
#     for ASD_p in ['df_feature_lowMinimal_CSS','df_feature_moderate_CSS','df_feature_high_CSS']:
#         FeatureLabelPair=[f'TD vs {ASD_p} >> {singleFeat}','ASDTD']
#         FeatureLabelMatch_manual.append(FeatureLabelPair)

# opt2
FeatureLabelMatch_manual=[
['TD vs df_feature_lowMinimal_CSS >> LOCDEP_Proximity_cols', 'ASDTD'],
['TD vs df_feature_moderate_CSS >> LOCDEP_Proximity_cols', 'ASDTD'],
['TD vs df_feature_high_CSS >> LOCDEP_Proximity_cols', 'ASDTD']
  ]
...
for exp_str,lab_ in FeatureLabelMatch:
    ...
    if len(FeatureLabelMatch_manual)==0:
        ...
        featuresOfInterest=featuresOfInterest_manual
    
    # feat_=key
    for feat_col in featuresOfInterest:
        feat_col_ = feat_col # ex: ['MSB_f1']
        ...
        Item_name="{exp_str}--{feat}::{lab}".format(exp_str='-'.join([exp_str]),feat=feat_col_,lab=lab_)
...
# label_choose=lab_
# pickle_path=''
# featuresOfInterest=feat_col_
# filterbyNum=False
# kwargs['feat_']=ados_ds.Features_comb_multi[exp_str]
# def Get_FormantAUI_feat(self,label_choose,pickle_path,featuresOfInterest=['MSB_f1','MSB_f2','MSB_mix'],filterbyNum=True,**kwargs):
#     arti=articulation.articulation.Articulation()
#     #如果path有放的話字串的話，就使用path的字串，不然就使用「feat_」等於的東西，在function裡面會以kwargs的形式出現
#     if not kwargs and len(pickle_path)>0:
