...
utts_all=[os.path.basename(file) for file in files_sorted]
names_all=set([utts[:re.search("_[D|K]",utts).start()] for utts in utts_all])


keys=[]
for name in names_all:
    files_people=[file for file in  np.array(files_sorted) if name in file]
    keys.append(files_people)

flat_keys=[item for sublist in keys for item in sublist]
assert len(flat_keys) == len(files)

multi=Multiprocess.Multi(filepath, MaxnumForm=5, AVERAGEMETHOD=AVERAGEMETHOD)
multi._updatePhonemapp(PhoneMapp_dict)
...
final_results=pool.starmap(multi.process_audio, [([file_block,silence,trnpath,PoolFormantWindow, True]) for file_block in tqdm(keys)])

Formants_utt_symb=Dict()
Wav_collect=Dict()
Trn_collect=Dict()
for load_file_tmp, _, [wavcolect, trncolect] in final_results:        
    for utt, df_phone in load_file_tmp.items():
        Formants_utt_symb[utt]=df_phone
    for spkr_name in wavcolect.keys():
        Wav_collect[spkr_name]=wavcolect[spkr_name]
        Trn_collect[spkr_name]=trncolect[spkr_name]
...
for utt in Formants_utt_symb.keys():
    ...
    # for symb in PhoneOI:
    outpath_ffl="{root}/{people}/".format(root=outputpath_filefilter,people=people)
...
PeopleOfInterest=AUI_info.keys()
# =============================================================================
Name2num=Dict()
for i,k in enumerate(sorted(AUI_info.keys())):
    Name2num[k]=i
...
for people in PeopleOfInterest:
    ...
    for symb in AUI_info[people].keys():
        df_tmp=AUI_info[people][symb]
        df_tmp['phone']=symb
